...
class SmsTransactionParser @Inject constructor() {
    ...
    fun parse(sender: String, body: String, receivedTimestamp: Long): ParsedTransaction? {
        if (!isTransactionalSms(sender, body)) {
            Log.d(TAG, "Not transactional SMS - sender='$sender', hasAmount=${amountPattern.containsMatchIn(body)}")
            return null
        }

        val amount = extractAmount(body)
        if (amount == null) {
            Log.d(TAG, "Could not extract amount from SMS")
            return null
        }
        val type = determineTransactionType(body)
        if (type == TransactionType.CREDIT) {
            Log.d(TAG, "Skipping credit/refund SMS")
            return null
        }
